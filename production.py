...
if uploaded_file is not None:
    ...
    first_recent_date = most_recent_date.iloc[0]   

    friday_df = most_recent_date[most_recent_date['DayOfWeek'] == 'Friday']
    friday = friday_df['GROSS PREMIUM'].sum()
    saturday_df = most_recent_date[most_recent_date['DayOfWeek'] == 'Saturday']
    saturday = saturday_df['GROSS PREMIUM'].sum()
    sunday_df = most_recent_date[most_recent_date['DayOfWeek'] == 'Sunday']
    sunday = sunday_df['GROSS PREMIUM'].sum()
    
    if first_recent_date.weekday() == 4:
        yesterday = friday
    elif first_recent_date.weekday() == 5:
        yesterday = (friday + saturday)
    elif first_recent_date.weekday() == 6:
