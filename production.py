...
theme_neutral = {'bgcolor': '#e8d5b7','title_color': 'orange','content_color': '#222831','icon_color': 'orange', 'icon': 'bi bi-battery-half'}
...
if uploaded_file is not None:
    ...
    jointdf['NEW TM'] = jointdf['NEW TM'].fillna(jointdf['TM'])
                ...
                with card_container(key="chart4"):
                    ...
                    with tab1:
                        with card_container(key="yesterday_tabs"):
                            ...
                            fig.update_layout(title={'text': 'YESTERDAY ACCOUNT MANAGERS PRODUCTION SUMMARY', 'x': 0.5, 'xanchor': 'center'},  xaxis=dict(tickfont=dict(size=10)), width=975) 
    ...
    if view == 'Territorial Manager':
        ...
        if first_recent_date.iloc[0].weekday() == 4:
            ...
            yesterday_receipts_total = most_recent.loc[most_recent['RECEIPTS'] > 0, 'RECEIPTS'].sum()
        ...
        week_cancelled = filtered_data[filtered_data['GROSS PREMIUM'] < 0]
